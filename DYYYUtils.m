...
static NSString *DYYYMediaInfoSequenceSuffix(NSDictionary *mediaInfo) {
    id sequenceValue = mediaInfo[@"sequenceIndex"];
    NSInteger sequenceIndex = [sequenceValue respondsToSelector:@selector(integerValue)] ? [sequenceValue integerValue] : 0;
    return sequenceIndex > 0 ? [NSString stringWithFormat:@"_%ld", (long)sequenceIndex] : @"";
}

static NSString *DYYYMediaInfoFilenameTimestamp(NSNumber *timestamp) {
    return DYYYMediaInfoFormattedTimestamp(timestamp);
}
...
static NSString *DYYYMediaInfoSummary(NSDictionary *mediaInfo) {
    ...
    NSString *sequenceSuffix = DYYYMediaInfoSequenceSuffix(mediaInfo);
    NSString *displayPublishTime = [NSString stringWithFormat:@"%@%@",
...
static NSString *DYYYMediaInfoFilename(NSDictionary *mediaInfo, NSString *pathExtension) {
    ...
    NSString *sequenceSuffix = DYYYMediaInfoSequenceSuffix(mediaInfo);
    NSString *resolvedPublishTime = [NSString stringWithFormat:@"%@%@",
                                                       publishTime.length > 0 ? publishTime : @"未知",
                                                       sequenceSuffix];
