...
@property(nonatomic, strong) NSMutableDictionary<NSString *, NSDictionary *> *livePhotoMediaInfoMap;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _livePhotoMediaInfoMap = [NSMutableDictionary dictionary];
...
+ (void)downloadLivePhoto:(NSURL *)imageURL videoURL:(NSURL *)videoURL completion:(void (^)(void))completion {
    [self downloadLivePhoto:imageURL videoURL:videoURL awemeModel:nil completion:completion];
}
...
+ (void)downloadLivePhoto:(NSURL *)imageURL
    ...
    NSDictionary *mediaInfo = [self mediaInfoFromAwemeModel:awemeModel];
    if (mediaInfo.count > 0) {
        manager.livePhotoMediaInfoMap[uniqueKey] = mediaInfo;
    }
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            if (imageExists && videoExists) {
                NSDictionary *cachedMediaInfo = manager.livePhotoMediaInfoMap[uniqueKey];
                [[DYYYManager shared] saveLivePhoto:imagePath videoUrl:videoPath mediaInfo:cachedMediaInfo];
                if (completion) {
                    completion();
                }
                return;
      ...
      dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        if (downloadSucceeded) {
            @try {
                ...
                [manager.livePhotoMediaInfoMap removeObjectForKey:uniqueKey];
...
- (void)saveLivePhoto:(NSString *)imageSourcePath videoUrl:(NSString *)videoSourcePath {
    [self saveLivePhoto:imageSourcePath videoUrl:videoSourcePath mediaInfo:nil];
}
...
- (NSString *)livePhotoMetadataSummaryFromMediaInfo:(NSDictionary *)mediaInfo {
    if (![mediaInfo isKindOfClass:[NSDictionary class]] || mediaInfo.count == 0) {
        return nil;
    }
    NSString *shortID = [mediaInfo[@"shortID"] isKindOfClass:[NSString class]] ? mediaInfo[@"shortID"] : @"未知";
    NSString *nickname = [mediaInfo[@"nickname"] isKindOfClass:[NSString class]] ? mediaInfo[@"nickname"] : @"未知用户";
    NSNumber *createTime = [mediaInfo[@"createTime"] respondsToSelector:@selector(doubleValue)] ? mediaInfo[@"createTime"] : nil;
    NSTimeInterval seconds = (createTime.doubleValue > 0) ? createTime.doubleValue : [[NSDate date] timeIntervalSince1970];
    ...
    NSDateComponents *components = [calendar components:(NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond)
                                               fromDate:date];
    NSString *timeText = [NSString stringWithFormat:@"%04ld-%02ld-%02ld %02ld:%02ld:%02ld",
    ...
    return [NSString stringWithFormat:@"抖音号: %@ · 抖音用户: %@ · 发布时间: %@", shortID, nickname, timeText];
}
...
- (void)useAssetWriter:(NSURL *)photoURL
    ...
    [self addMetadataToPhoto:photoURL outputFile:photoFile identifier:identifier mediaInfo:mediaInfo];
...
- (void)addMetadataToPhoto:(NSURL *)photoURL outputFile:(NSString *)outputFile identifier:(NSString *)identifier mediaInfo:(NSDictionary *)mediaInfo {
    NSMutableData *data = [NSData dataWithContentsOfURL:photoURL].mutableCopy;
    UIImage *image = [UIImage imageWithData:data];
    CGImageRef imageRef = image.CGImage;
    NSString *summary = [self livePhotoMetadataSummaryFromMediaInfo:mediaInfo];
    NSMutableDictionary *imageMetadata = [@{(NSString *)kCGImagePropertyMakerAppleDictionary : @{@"17" : identifier}} mutableCopy];
    if (summary.length > 0) {
        imageMetadata[(NSString *)kCGImagePropertyTIFFDictionary] = @{(NSString *)kCGImagePropertyTIFFImageDescription : summary};
        imageMetadata[(NSString *)kCGImagePropertyExifDictionary] = @{(NSString *)kCGImagePropertyExifUserComment : summary};
    }
              ...
              for (NSDictionary *fileInfo in downloadedFiles) {
                  ...
                  if (![imagePath isKindOfClass:[NSNull class]] && ![videoPath isKindOfClass:[NSNull class]] && [fileManager fileExistsAtPath:imagePath] && [fileManager fileExistsAtPath:videoPath]) {
                      ...
                      dispatch_async(processQueue, ^{
                        ...
                        [[DYYYManager shared] addMetadataToPhoto:[NSURL fileURLWithPath:imagePath] outputFile:photoFile identifier:identifier mediaInfo:nil];
