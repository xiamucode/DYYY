...
@interface DYYYLegacyVideoSaveHelper : NSObject
@property(nonatomic, copy) void (^completion)(BOOL success);
+ (void)saveVideoAtURL:(NSURL *)videoURL completion:(void (^)(BOOL success))completion;
@end

@implementation DYYYLegacyVideoSaveHelper

+ (NSMutableSet<DYYYLegacyVideoSaveHelper *> *)activeHelpers {
    static NSMutableSet<DYYYLegacyVideoSaveHelper *> *helpers = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      helpers = [NSMutableSet set];
    });
    return helpers;
}
...
+ (void)saveVideoAtURL:(NSURL *)videoURL completion:(void (^)(BOOL success))completion {
    ...
    DYYYLegacyVideoSaveHelper *helper = [[DYYYLegacyVideoSaveHelper alloc] init];
    helper.completion = completion;

    @synchronized([self activeHelpers]) {
        [[self activeHelpers] addObject:helper];
    }

    dispatch_async(dispatch_get_main_queue(), ^{
      UISaveVideoAtPathToSavedPhotosAlbum(videoURL.path, helper, @selector(video:didFinishSavingWithError:contextInfo:), NULL);
    });
}
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    @synchronized([DYYYLegacyVideoSaveHelper activeHelpers]) {
        [[DYYYLegacyVideoSaveHelper activeHelpers] removeObject:self];
    }
}
              ...
              completionHandler:^(BOOL success, NSError *_Nullable error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                  if (!success && finalMediaType == MediaTypeVideo) {
                      [DYYYLegacyVideoSaveHelper saveVideoAtURL:finalURL
