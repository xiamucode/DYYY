...
+ (void)saveMedia:(NSURL *)mediaURL
    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
      ...
      void (^persistProcessedMedia)(NSURL *, MediaType) = ^(NSURL *finalURL, MediaType finalMediaType) {
          [[PHPhotoLibrary sharedPhotoLibrary]
              performChanges:^{
      ...
      if (mediaInfo.count > 0) {
          if (mediaType == MediaTypeImage) {
              UIImage *image = [UIImage imageWithContentsOfFile:mediaURL.path];
              UIImage *decoratedImage = [DYYYUtils imageByAppendingMediaInfo:mediaInfo toImage:image];
              if (decoratedImage) {
                  NSString *extension = mediaURL.pathExtension.lowercaseString;
                  NSData *imageData = [extension isEqualToString:@"png"] ? UIImagePNGRepresentation(decoratedImage) : UIImageJPEGRepresentation(decoratedImage, 0.95);
                  if (imageData.length > 0 && [imageData writeToURL:mediaURL atomically:YES]) {
                      persistProcessedMedia(mediaURL, mediaType);
                      return;
                  }
              }
          } else if (mediaType == MediaTypeVideo) {
              [DYYYUtils appendMediaInfo:mediaInfo
...
+ (void)downloadMediaWithProgress:(NSURL *)url
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      [[DYYYManager shared] setMediaInfo:[self mediaInfoFromAwemeModel:awemeModel] forDownloadID:downloadID];
